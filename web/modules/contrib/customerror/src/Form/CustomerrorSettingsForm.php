  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    foreach ($errors as $code => $desc) {
      if (\Drupal::config('system.site')->get("page.$code") != "/customerror/{$code}") {
        $this->messenger()->addMessage($this->t('Custom error is not configured for @error errors. Please ensure that the default @error page is set to be /customerror/@error on the @link.', ['@error' => $code, '@link' => Link::createFromRoute($this->t('Site information settings page'), 'system.site_information_settings')->toString()]), 'error', FALSE);
      }
    }
    foreach ($errors as $code => $desc) {
      $form[$code] = [
